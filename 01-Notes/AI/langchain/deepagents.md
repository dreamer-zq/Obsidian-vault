...
- 🔄 **自动可用** - 无需配置,默认就存在
- 🎯 **继承主代理配置** - 使用相同的 system prompt、tools、model
- 🧹 **仅用于上下文隔离** - 不提供专业化能力,只是为了保持主代理上下文清洁
- ⚡ **适合临时任务** - 用于处理不需要特殊指令的多步骤任务
...
- ⚙️ **需要显式配置** - 需要定义 name、description、tools、prompt 等
- 🎨 **专业化定制** - 可以有完全不同的 system prompt、专用工具集、甚至不同的模型
- 🎯 **领域专家** - 提供针对特定任务的专业指令和能力
- 🔧 **完全控制** - 可以提供整个 LangGraph graph (CompiledSubAgent)
...
| 特性              | General-Purpose Subagent | 自定义子代理                           |
| ----------------- | ------------------------ | -------------------------------------- |
| **配置要求**      | 无需配置,自动可用        | 需要显式定义 SubAgent/CompiledSubAgent |
| **System Prompt** | 与主代理相同             | 可完全自定义                           |
| **工具集**        | 与主代理相同             | 可使用专用工具或主代理工具的子集       |
| **模型**          | 与主代理相同 (可覆盖)    | 可使用不同模型                         |
| **主要用途**      | 上下文隔离               | 专业化任务处理                         |
| **复杂度**        | 低                       | 中到高                                 |
| **调用方式**      | 通过 task 工具自动       | 通过 task 工具,需指定 subagent 名称    |
...
问题:主代理需要委托一个任务
    ...
    │   └─ 否 ↓
    │
    ├─ 主要目的是隔离上下文和避免工具调用污染吗?
    │   ├─ 是 → 使用 general-purpose subagent ✓
    │   └─ 否 ↓
    │
    └─ 默认 → 使用 general-purpose subagent ✓
```
