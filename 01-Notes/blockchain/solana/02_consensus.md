...
```mermaid
graph LR
    A["区块 N-1<br/>Hash_N-1<br/>Tick_1<br/>Tick_2<br/>...<br/>Tick_k"] 
    B["区块 N<br/>Hash_N<br/>Tick_1<br/>Tick_2<br/>...<br/>Tick_k"]
    C["区块 N+1<br/>Hash_N+1<br/>Tick_1<br/>Tick_2<br/>...<br/>Tick_k"]
...
sequenceDiagram
    ...
    participant A as 验证者 A
    participant B as 验证者 B
    participant C as 验证者 C
    
    Note over L,C: Tower BFT 投票过程
    
    L->>A: 1. 提议区块
    L->>B: 1. 提议区块
    L->>C: 1. 提议区块
    
    Note over A,C: 2. 验证区块并投票
    A->>L: 投票确认
    B->>L: 投票确认
    C->>L: 投票确认
    
    Note over L: 3. 收集投票<br/>4. 达到阈值后确认
...
```mermaid
graph TD
    subgraph "Leader 选择过程"
        T1["时间槽 1<br/>Leader A<br/>(权重30%)"]
        T2["时间槽 2<br/>Leader B<br/>(权重25%)"]
        T3["时间槽 3<br/>Leader C<br/>(权重20%)"]
        T4["时间槽 4<br/>Leader A<br/>(权重30%)"]
...
graph TD
    subgraph "网络分区处理流程"
        A["正常状态<br/>完整网络<br/>所有节点正常通信"]
        
        B1["分区 A<br/>(权重 60%)<br/>继续出块"]
        B2["分区 B<br/>(权重 40%)<br/>停止出块"]
        
        C["恢复状态<br/>网络恢复<br/>分区 B 同步分区 A 的状态"]
