...
```mermaid
flowchart TD
    A[Mempool 交易池] --> B[Sequencer 选择交易]
    B --> C[StateKeeper 初始化执行环境]
    C --> D[Bootloader 批量执行]
    D --> E[EraVM 逐笔处理]
    E --> F[Witness Tracer 记录轨迹]
    F --> G[状态更新 & 日志生成]
    G --> H[计算 stateRoot & logsRoot]
    H --> I[生成 Miniblock]
...
> **注意**：通常没有交易时不会产生 Miniblock；出块策略可配置（时间心跳或按需）。但是在batch生成时，bootloader 会强制产生一个空的miniblock，该空区块包含一个转账事件日志，代表 bootloader 将收取的费用转给运营商。
...
graph LR
    subgraph "L2 Layer"
        MB1[Miniblock 1<br/>Txs: 50<br/>State Changes: Δ1]
        MB2[Miniblock 2<br/>Txs: 75<br/>State Changes: Δ2]
        MB3[Miniblock 3<br/>Txs: 25<br/>State Changes: Δ3]
    end
    
    subgraph "Batch Aggregation"
        BA[L1 Batch<br/>State Diffs: Δ1+Δ2+Δ3<br/>Pubdata: 128KB<br/>Commitment: Hash]
    end
    
    subgraph "Off-chain Processing"
        P[Prover System<br/>基于执行 trace<br/>生成 ZK 证明]
    end
    
    subgraph "L1 Layer"
        CC[Commit Contract<br/>存储状态差异 & 承诺]
        PC[Prove Contract<br/>验证 ZK 证明]
        EC[Execute Contract<br/>执行 Batch 并最终确认]
    end
...
sequenceDiagram
    ...
    Note over BA: Batch 封闭（状态差异聚合）
    Note over P: 并行：基于执行 trace<br/>生成 ZK 证明
    
    rect rgb(240, 248, 255)
        Note over CC, EC: L1 Layer - 三阶段处理
        
        rect rgb(255, 243, 224)
            Note over CC: 阶段 1: Commit
            BA->>CC: commitBatches(pubdata, commitment)
            Note over CC: 存储状态差异 & 承诺
        end
        
        rect rgb(227, 242, 253)
            Note over PC: 阶段 2: Prove
        ...
        rect rgb(252, 228, 236)
            Note over EC: 阶段 3: Execute
            Note over EC: 前置条件：Commit + Prove 完成
            BA->>EC: executeBatches()
            Note over EC: 更新状态根<br/>处理提款/消息<br/>标记 Batch 为已执行
        end
    end
...
stateDiagram-v2
    [*] --> Received: 用户提交
    Received --> Validated: 基础验证
    Validated --> Queued: 进入 Mempool
    Queued --> Sequenced: Sequencer 选择
    Sequenced --> Executing: Bootloader 执行
    Executing --> Executed: EraVM 完成
    Executed --> Included: 写入 Miniblock
    Included --> Committed: Batch 提交 L1
    Committed --> Proved: ZK 证明验证
    Proved --> Finalized: L1 执行确认
    
    Validated --> Rejected: 验证失败
    Executing --> Failed: 执行失败
    Rejected --> [*]
    Failed --> [*]
    
    note right of Included: L2 软确认<br/>用户可见
    note right of Committed: 数据可用性<br/>保证
    note right of Proved: 安全性<br/>保证
    note right of Finalized: 最终确认<br/>不可回滚
```
...
sequenceDiagram
    ...
    U->>L2B: 调用 withdraw()
    L2B->>L1M: 生成 L2→L1 消息
    Note over L1M: 消息包含在 Batch 中
    Note over L1: Batch 被证明并执行
    U->>L1B: 调用 finalizeWithdrawal()
    L1B->>L1: 验证消息证明
    L1-->>U: 资金到账
```
...
```mermaid
graph TD
    subgraph "Pubdata 发布方式"
        CD[Calldata<br/>~16 gas/byte]
        BLOB[EIP-4844 Blob<br/>~1 gas/byte]
    end
    
    subgraph "数据类型"
        SD[状态差分<br/>State Diffs]
        LD[日志数据<br/>L2→L1 Logs]
        MD[消息数据<br/>Cross-layer Messages]
    end
...
graph TD
    subgraph "最终性层级"
        L2[L2 软确认<br/>~1-10秒<br/>用户体验优化]
        L1C[L1 承诺<br/>~1-10分钟<br/>数据可用性保证]
        L1P[L1 证明<br/>~1-24小时<br/>密码学安全保证]
        L1E[L1 执行<br/>~1-24小时<br/>最终不可回滚]
    end
...
sequenceDiagram
    ...
    participant S as Sequencer
    participant L2 as L2 网络
    
    Note over S: Sequencer 审查用户交易
    U->>L1: 通过 L1 强制包含
    L1->>PQ: 添加到优先队列
    Note over PQ: 等待处理（最长 ~24小时）
    S->>PQ: 必须处理优先交易
    S->>L2: 执行强制包含的交易
    L2-->>U: 交易成功执行
```
...
> 💡 **下一步阅读**：[05_Prividium私有链](./05_Prividium.md) - 深入了解Prividium私有链
