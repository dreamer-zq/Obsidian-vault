...
> **前置阅读**:
>
> - [总体架构](./01_总体架构.md) - 了解整体设计理念
> - [核心模块分析](./02_核心模块分析.md) - 掌握模块关联关系
>
> **后续阅读**: [04_区块和交易](./04_区块和交易.md) - 企业应用案例
...
EraVM (Zero-Knowledge Ethereum Virtual Machine) 是 zkSync Era 的执行引擎，通过 EVM Interpreter（v27）解释执行未修改的 EVM 字节码，并生成零知识证明验证执行正确性；仍存在少量操作码差异与限制。

#### A. 设计目标

- **EVM 字节码支持**: 通过 EVM Interpreter（v27）执行未修改的 EVM 字节码；存在少量操作码差异与限制（如 CALLCODE、SELFDESTRUCT、BLOBHASH、BLOBBASEFEE），禁止跨 EraVM/EVM delegatecall；Gas 为 EraVM ergs；预编译保持兼容。
...
- **缓存模块**: Mempool - 交易池管理
- **证明系统**: Prover System - ZK proof 生成s

### 2.3 兼容性特性

#### A. Solidity 兼容性

- **编译器支持**: 支持标准 Solidity 源码（Solidity 语言级兼容），但必须通过 zksolc 编译为 EraVM 字节码后部署；EraVM 对 EVM 操作码保持高兼容性，但部分指令（如 SELFDESTRUCT、CALLCODE）被禁用。
- **ABI 兼容**: 完全兼容以太坊 ABI 标准
- **库支持**: 支持 OpenZeppelin 等标准库

#### B. 工具链兼容性

- **Hardhat**: 支持 Hardhat 开发框架；特性依赖网络配置与插件 （Hardhat 完全兼容，但必须使用 ZKsync 的插件版本，非原生 hardhat-ethers 那种直接配置）
- **Foundry**: 支持 Foundry 测试和部署
- **Remix**: 兼容 Remix IDE

#### C. 协议兼容性

- **EIP 支持**: 支持主流 EIP；不支持与 4844 blob 交互相关的执行环境指令（如 `BLOBHASH`、`BLOBBASEFEE`， 原因：ZKsync 的数据可用性 (DA) 模型目前并非依赖以太坊 blob，而是自己的压缩 calldata + Validium / EigenDA 方案）
- **预编译合约**: 支持标准预编译合约（与以太坊保持接口兼容）
- **计费模型**: 采用 EraVM `ergs` 计费；与以太坊 Gas 不同，但接口层保持兼容（API 映射）
- **差异与限制**: 不支持 `CALLCODE`、`SELFDESTRUCT`；禁止跨 EraVM/EVM 的 `delegatecall`

### 2.4 系统合约

zkSync Era 的系统合约分为 L1 合约和 L2 合约两个层面，它们共同构成了 zkStack/zkSync 系统合约生态。
...
**主要功能**: CTM 管理链类型与升级策略，并协调各链的验证器配置。
...
- **验证器配置**: Proof 验证由各链的 L1 Diamond/Executor 调用共享 Verifier 完成；CTM 统一管理验证密钥与版本
- **升级管理**: 强制所有链更新到最新实现版本
...
- **失败恢复**: 提供失败存款的链上恢复机制
- **基础代币支持**: 负责在不同链之间路由各链的基础代币（Base Token）桥接

**安全机制**:

- **白名单验证**: 验证代币和操作的合法性
- **失败交易恢复**: 提供 `bridgeRecoverFailedTransfer` 功能
- **治理保护**: 关键变更通过治理合约的时间锁与安全委员会流程管理
...
**主要功能**: Bootloader 是 zkSync Era 的核心执行协调器，负责管理和执行所有类型的交易并生成执行轨迹（witness），供证明系统使用。
...
- **Ergs Accounting**: 详细的 ergs 使用统计信息
...
- “在 Bootloader 中执行批次的交易” 是指受控的标准化执行环境重演批次内的所有交易，以确保一致性和正确性，并生成可验证的执行轨迹（witness）；实际的证明由链下 Prover 系统生成。
...
3. **消息编码**: 将消息内容编码为标准格式
4. **哈希计算**: 计算消息的唯一哈希标识
5. **事件记录**: 生成 L2→L1 日志事件，记录消息详情
6. **状态更新**: 更新消息发送状态和计数器

**输出数据 (下游系统)**:

- **Event Log System**: L2→L1 日志事件，包含消息哈希和内容
- **Merkle/日志根构建**: 消息哈希用于构建 Merkle 树或日志根证明
...
这些系统合约通过精心设计的接口和数据流，确保 zkSync Era/zkStack 各项功能安全高效运行，同时在 EraVM 通过 EVM Interpreter 支持未修改的 EVM 字节码，保持与以太坊生态的高度兼容性。
...
**数据存储**: 在 L1 发布用于重构状态的 pubdata（状态差分），通过 `calldata` 或 `EIP‑4844 blob`
**安全性**: 最高 - 继承以太坊的安全性
**成本**: 较高 - 需要支付 L1 数据发布费用
**数据可用性**: 公开可用 - 任何人可从 L1 pubdata 重建 L2 状态（`calldata` 永久、`blob` 依赖长期存储策略与采样保障）
...
- **EraVM**: 通过 EVM Interpreter 支持未修改 EVM 字节码；存在少量操作码差异与限制
- **Prover**: 高效的零知识证明系统
- **Node架构**: 模块化的服务器组件
- **DA策略**: 灵活的数据可用性选择
