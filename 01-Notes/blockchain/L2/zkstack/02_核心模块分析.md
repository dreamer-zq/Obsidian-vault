...
>
> **前置阅读**: [总体架构](./01_总体架构.md) - 建议先了解整体架构设计
>
> **后续阅读**:
>
...
- **系统合约**:
  ...
  - L1Messenger
- **关键特性**:
  - 通过 EVM Interpreter（v27）执行未修改的 EVM 字节码；存在少量操作码差异与限制（如 CALLCODE、SELFDESTRUCT、BLOBHASH、BLOBBASEFEE），禁止跨 EraVM/EVM delegatecall；Gas 为 EraVM ergs。
  - 高效的证明生成
  - 系统级优化
...
- **Sequencer (排序器)**:
  ...
  - 批次构建
  - MEV 保护
  注：ZKsync Era 主网当前采用中心化排序器，去中心化排序为路线图方向。
- **State Keeper (状态保持器)**:
  - 状态管理
  - 执行协调
  - 一致性维护
...
- **Rollup 模式**:
  - 在 L1 发布用于重构状态的 pubdata（状态差分），可通过 calldata 或 EIP‑4844 blob
  - 最高级别的数据可用性，任何人可从 L1 pubdata 重建 L2 状态
  - 高安全性，高成本
- **Validium 模式**:
  - 数据存储在链下
  - 数据可用性委员会
