...
> 2. [核心模块分析](./01_核心模块分析.md) - 掌握重点模块和关联关系
> 3. [核心组件详解](./03_核心组件.md) - 深入理解技术实现
> 4. [Prividium 企业级隐私区块链](./04_Prividium.md) - 企业应用案例

## 1. 架构概览

zkSync Era 是一个基于零知识证明的以太坊 Layer 2 扩容解决方案，采用了创新的弹性网络架构。其设计目标是在保持以太坊安全性的同时，提供高吞吐量、低成本的交易处理能力。
...
- **Rollup 模式**: 交易数据完全发布到以太坊 L1，提供最高安全保证
...
2. **资产锁定**: BridgeHub 智能合约锁定用户的资产
3. **事件生成**: 生成包含存款信息的事件和 Merkle 证明
4. **L2 监听**: L2 的 ETH Watcher 组件监听并捕获 L1 事件
5. **证明验证**: L2 验证 Merkle 证明的有效性
6. **代币铸造**: 验证通过后，在 L2 铸造对应数量的代币
...
6. **等待确认**: 提取的最终完成时间依赖于 prover 生成 proof 并在 L1 提交/验证的延迟，时间范围从几十秒到数小时不等（取决于证明系统、聚合策略与提交频率）。
